...
# Tên riêng trên Prompt Hub để không trùng với người dùng khác.
PROMPT_V1_NAME = "bach-rag-v1"   # ví dụ: "nguyen-rag-v1"
PROMPT_V2_NAME = "bach-rag-v2"   # ví dụ: "nguyen-rag-v2"


# ── 2. Định nghĩa 2 Prompt Templates ──────────────────────────────────────
# V1: phong cách thân thiện, ngắn gọn.
SYSTEM_V1 = (
    "Bạn là trợ lý AI thân thiện. Chỉ trả lời dựa trên context được cung cấp. "
    "Trả lời ngắn gọn, rõ ràng trong 2–4 câu. Nếu context không có thông tin, "
    "hãy nói thẳng là không biết.\n\nContext:\n{context}"
)
...
# V2: phong cách chuyên gia, trả lời có cấu trúc.
SYSTEM_V2 =  (
    "Bạn là chuyên gia phân tích thông tin. Chỉ sử dụng context được cung cấp, "
    "không suy đoán thêm. Trả lời có cấu trúc gồm: "
    "1) Kết luận chính, 2) Căn cứ từ context, 3) Mức độ chắc chắn. "
    "Nếu context không đủ, hãy nêu rõ điều đó.\n\nContext:\n{context}"
)
...
def push_prompts_to_hub(client: Client):
    ...
    try:
        url = client.push_prompt(
            PROMPT_V1_NAME,
            object=PROMPT_V1,
            description="V1: Phong cách thân thiện, ngắn gọn (2–4 câu).",
        )
    ...
    try:
        url = client.push_prompt(
            PROMPT_V2_NAME,
            object=PROMPT_V2,
            description="V2: Phong cách chuyên gia, có cấu trúc và mức độ chắc chắn.",
        )
