...
def setup_vectorstore():
    ...
    """
    # 1) Khởi tạo embedding model theo PROVIDER trong .env
    embeddings = get_embeddings()

    # 2) Đọc nội dung knowledge base (data/knowledge_base.txt)
    text = load_knowledge_base()

    # 3) Chia text thành các chunks nhỏ để index
    chunks = split_text(text, chunk_size=500, chunk_overlap=50)
...
RAG_PROMPT = ChatPromptTemplate.from_messages([
    (
        ...
        "Nếu context không chứa thông tin cần thiết, hãy nói "
...
def build_rag_chain(vectorstore):
    ...
    # Retriever lấy 3 chunks gần nhất với câu hỏi
...
def main():
    ...
    print(f"\n🚀 Đang chạy {len(SAMPLE_QUESTIONS)} câu hỏi qua RAG pipeline ...\n")
